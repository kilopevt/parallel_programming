...
3. Для каждого размера и количества потоков (1, 2, 4, 8) запустите программу, указав пути к соответствующим файлам матриц(матрицы взяты из предыдущей          лабораторной работы) и имя выходного CSV-файла(например, C1_200.csv). Каждый замер повторите 3 раза для получения среднего значения.
4. Пример:
    ```
    matrix_omp.exe
